...
export default class ActorSheetFlags extends BaseEntitySheet {
  static get defaultOptions() {
    ...
    return mergeObject(options, {
      id: "actor-flags",
	    classes: ["dnd5e"],
      template: "systems/sw5efoundry/templates/apps/actor-flags.html",
      width: 500,
      closeOnSubmit: true
    });
  }
  ...
  _getFlags() {
    ...
    for ( let [k, v] of Object.entries(CONFIG.DND5E.characterFlags) ) {
      ...
      flag.value = getProperty(baseData.flags, `sw5efoundry.${k}`);
      flags[v.section][`flags.sw5efoundry.${k}`] = flag;
    }
    return flags;
  }
  ...
  async _updateObject(event, formData) {
    ...
    let unset = false;
    const flags = updateData.flags.sw5efoundry;
    for ( let [k, v] of Object.entries(flags) ) {
      if ( [undefined, null, "", false, 0].includes(v) ) {
        delete flags[k];
        if ( hasProperty(actor._data.flags, `sw5efoundry.${k}`) ) {
          unset = true;
          flags[`-=${k}`] = null;
        }
      }
